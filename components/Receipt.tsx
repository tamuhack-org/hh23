...
const Receipt = ({ day, scheduleItems }: ReceiptProps) => {
  ...
  return (
    ...
    >
      <motion.div
        className={`${spaceMono.className} relative flex flex-col items-center justify-center bg-receipt bg-cover max-[571px]:w-full max-[1200px]:w-[480px] w-[550px] overflow-clip p-8 lg:p-10 gap-4 my-4 lg:my-0 lg:mx-4`}
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        transition={{ duration: 0.5, delay: 0.3 }}
        viewport={{ once: true }}
      >
        ...
        <h1 className={`${archivo.className} font-bold text-3xl mb-2 lg:mb-4`}>
          {day}
        </h1>
        ...
        <div className="flex flex-row justify-between items-center text-xs lg:text-sm font-bold w-full text-left gap-2 lg:gap-4 select-none">
          <h2 className="w-[30%] lg:w-[20%]">TIME</h2>
          <h2 className="w-[70%] lg:w-[80%]">EVENT</h2>
        </div>
        {scheduleItems
          .sort(
            (item1, item2) =>
          ...
          .map((scheduleItem, i) => (
            <div
              className="flex flex-row justify-between items-start border-b border-black border-opacity-40 text-xs lg:text-sm w-full text-left py-2 gap-2 lg:gap-4 uppercase hover:border-opacity-100 transition duration-200 select-none"
              key={i}
            >
      ...
      {day === "SUNDAY" && (
        ...
        >
          ...
          >
            {isRevHovered ? (
              <Image
                src="/assets/retro-rev-click.png"
                width={300}
                height={200}
                alt="retro-rev-click"
                className=""
